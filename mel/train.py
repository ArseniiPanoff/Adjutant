...
NUM_EPOCHS = 40  # More epochs to give the model time to learn
...
def train():
    ...
    attention = Attention(hidden_dim=HIDDEN_DIM)  # Ensure this matches hidden_dim in your Decoder
    decoder = Decoder(hidden_dim=HIDDEN_DIM, output_dim=OUTPUT_DIM, attention=attention,
                      max_len=MAX_SPECTROGRAM_LENGTH).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for mel_spectrograms, transcriptions in dataloader:
            ...
            optimizer.zero_grad()
            encoder_output, hidden = encoder(text_inputs)
            outputs, _, _ = decoder(mel_spectrograms, hidden, encoder_output)
